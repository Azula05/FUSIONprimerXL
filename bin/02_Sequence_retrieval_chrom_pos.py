...
"""
This script generates sequences based on the fastahack index generated in assembly, specified chromosomal positions of the breakpoint
togheter with the number of nucleotides that surround the breakpoint are used by FASTAhack to create the sequence.
The following files are generated:
- fasta_in.txt: contains the chromosomal positions of the breakpoint
- fasta_track.txt: corresponds witht the fasta_in.txt file, naming the sequences END (left part) and START (right part)
- annotation file: containing annotation information for the fusion RNA
The input should be a bed file with 0 based counting (required for checking the exons)
The following arguments are required:
- -n: the number of nucleotides surrounding the breakpoint
- -i: the input bed file with the fusion RNA
- -s: spliced? yes or no
- -e: bed file with canonical exons
- -t: list with canonical transcripts

Note: the left part is refered to as end and the right part as start
"""
...
# get info on BP seq
parser = argparse.ArgumentParser(description='give arguments to main nextflow script')
...
# add parameters for fusion annotation
annotation = open('annotation_' + fusion_ID + '.txt', 'w')
annotation.write(fusion_ID + "**")

# create files to write the sequences to
...
# check positions for same chromosome
if fusion_chrom1 == fusion_chrom2:
    if fusion_end > fusion_start:
        exit ("ERROR: END position of the left part is larger than START position of the right part for fusion on the same chromosome. Please check file.")
...
# B) spliced
    ...
    ## see if END pos BP is in exon
    # make file with theoretical left side of BP
    ...
    ## see if START pos is in exon
    # make file with theoretical right side of BP
    start_bed = open("start.bed", "w")
    start_bed.write(fusion_chrom2 + "\t" + str(fusion_start) + "\t" + str(fusion_start + temp_length) + "\n")
    start_bed.close()
    ...
    ############# if both in exon => do spliced
        ...
        ############################################################################################
            ...
            ########## STEP 5
                ...
                fasta_track.write(">end_" + str(fasta_end) + '\n')
                fasta.write(fusion_chrom1.replace("chr", "") + ":" + str(fusion_end + 1 - temp_length_end) + "-" + str(fusion_end + 1) + "\n") # change to 1-based system
                new_end = int(fusion_end - temp_length_end)
                ...
                while temp_length_end < temp_length:
                    ...
                    if len(new_exons_end_pos) == 0:
                        ...
                        ########## STEP 7
                        # make sure that exon is not the one that's already used for the START pos (this is the case for 2-exon fusion)
        ...
        ############################################################################################
            ...
            # if not, it needs to be recalculated
                ...
                fasta_track.write(">start_" + str(fasta_start) + '\n')
                fasta.write(fusion_chrom2.replace("chr", "") + ":" + str(fusion_start + 1) + "-" + str(fusion_start + temp_length_start) + "\n") # change to 1-based system
                new_start = int(fusion_start + temp_length_start)
                ...
                while temp_length_start < temp_length:
                    ...
                    if len(new_exons_start_pos) == 0:
                        ...
                        ########## STEP 7                    
                        # make sure that exon is not the one that's already used for the END pos
